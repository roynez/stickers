...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	if(!empty($sticker_id))
	{		
		$get_result = $data_obj->get_webp_sticker_by_id($sticker_id);
		...
		if($get_count == 1)
		{
			...
			if(unlink($upload_sticker_webp_path.$sticker_image))
			{
				$d_result = $data_obj->remove_webp_sticker_by_id($sticker_id);
				if($d_result == true)
				{
					echo "0";
				}
