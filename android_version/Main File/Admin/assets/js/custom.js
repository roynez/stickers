...
$(document).ready(function (e) {
	...
	// For adding Png Sticker
	
	$("#submit-png-sticker-form").on('submit',(function(e) {
		...
		else
		{
			...
			$.ajax({
			url: "controller/addPngStickerController.php",
			type: "POST", 
			data: new FormData(this),
			contentType: false,      
			cache: false,            
			...
			success: function(result)
				{
					...
					{
						// reset form fields
						$('#submit-png-sticker-form')[0].reset();
...
{	
	...
	else
	{
		...
		$.ajax({
			...
			url:"controller/getPngStickerController.php",
			success: function(result)
				{
					//alert(result);
...
{	
	...
	if (r == true) 
	{
		...
		$.ajax({
			type:"post",
			data:{sticker_id:id},
			url:"controller/removePngStickerController.php",
			success: function(result)
			{
				if(result == 0)
				{
