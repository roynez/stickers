...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	{
		$result = $data_obj->get_webp_sticker_by_sub_cate_id($sub_cate_id);
		...
		{	
			while($row = mysqli_fetch_assoc($result))
			{
				...
				$sticker_image = $link_sticker_webp_path.$row['sticker_image'];	

				$sticker_image_src = "'".$link_sticker_webp_path.$row['sticker_image']."'";
				...
				echo'<div id="sticker_'.$sticker_id.'" class="col-md-2 sticker-div-top">
						<div class="sticker-div">
							...
							<p class="sticker-p">
								<i class="fa fa-remove sticker-i-delete" onClick="deleteWebpSticker('.$stckr_id.');"></i>
							</p>
						</div>	
					</div>';
			}		
