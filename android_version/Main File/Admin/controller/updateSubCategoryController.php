...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	@$sub_cate_image = $_POST['sub_cate_image'];
	@$sub_cate_tray_image = $_POST['sub_cate_tray_image'];
	...
	if(!empty($category_id) && !empty($sub_cate_name) && !empty($sub_cate_id))
	{
		...
		if($count == 0)
		{
			...
			$sub_cate_image_src = "'".$link_subcategory_path.$sub_cate_image."'";
			...
			$result1 = $data_obj->update_sub_cate($sub_cate_id,$category_id,$sub_cate_name,$sub_cate_image,$sub_cate_tray_image);
			if($result1 == true)
			{
				...
				echo '<td>
							<img onclick="zoomImage('.$sub_cate_image_src.')" class="category-image" src="'.$link_subcategory_path.$sub_cate_image.'"/>
						</td>
						<td>'.$sub_cate_name.'</td>
						<td>'.$category_name.'</td>
						<td>'.$is_active.'</td>
