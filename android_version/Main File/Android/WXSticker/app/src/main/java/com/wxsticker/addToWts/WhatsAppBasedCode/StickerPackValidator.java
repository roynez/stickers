...
public class StickerPackValidator {
    ...
    private static Uri getStickerAssetUri(String identifier, String stickerName) {
        return new Uri.Builder().scheme(StickerContentProvider.CONTENT_SCHEME).authority(BuildConfig.APPLICATION_ID +".addToWts.WhatsAppBasedCode.StickerContentProvider").appendPath(StickerContentProvider.STICKERS_ASSET).appendPath("2").appendPath(stickerName).build();
    }
}
